...
const helpers = {
	...
	"parse": function ( req, context, mCb ) {
		try {
			req.soajs.log.debug( "parsing uploaded file ... " );
			let file = req.files.filename;
			if ( !file || Object.keys( file ).length === 0 ) {
				return mCb( { code: 498, msg: "Invalid or no template file uploaded!" } );
			}
			
			if ( !file.name ) {
				return mCb( { code: 498, msg: "Invalid or no template name!" } );
			}
			
			let currentDate = new Date().toISOString();
			let originalFileName = file.filename.replace( ".zip", "" ) + "_" + currentDate;
